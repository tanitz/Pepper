...
# STT: Thonburian Whisper CT2  |  AI: Google Gemini  |  TTS: Google Translate / edge-tts
...
# ── เลือกเสียง TTS (แก้ตรงนี้) ───────────────────────────────────────────────
# "google"    → Google Translate TTS
# "niwat"     → edge-tts: Niwat (ชาย)
# "premwadee" → edge-tts: Premwadee (หญิง)
TTS_MODE = "niwat"
...
def pick_next_song():
    ...
    if not os.path.isdir(SONG_DIR):
        print(f"[เพลง] ไม่พบโฟลเดอร์ {SONG_DIR}", flush=True)
        return None
    songs = [f for f in os.listdir(SONG_DIR) if f.lower().endswith(".mp3")]
    if not songs:
        print(f"[เพลง] ไม่มีไฟล์ mp3 ใน {SONG_DIR}", flush=True)
        return None
    if not _song_queue:
        _song_queue.extend(songs)
        random.shuffle(_song_queue)
        print(f"[เพลง] สุ่มรายการใหม่ {len(_song_queue)} เพลง", flush=True)
    chosen = _song_queue.pop(0)
    print(f"[เพลง] เล่น: {chosen}  (เหลือในคิว {len(_song_queue)} เพลง)", flush=True)
        ...
        if text:
            ...
            if re.search(r'\[SING\]', answer, re.IGNORECASE):
                ...
                if result:
                    ...
                    speak_text = "ขอโทษครับ ไม่พบเพลงในโฟลเดอร์ครับ"
                    if speak(speak_text):
                        if pepper_up:
                            write_status("busy")
